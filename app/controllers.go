...
func MountObservationController(service *goa.Service, ctrl ObservationController) {
	...
	h = func(ctx context.Context, rw http.ResponseWriter, req *http.Request) error {
		...
		// Build the payload
		if rawPayload := goa.ContextRequest(ctx).Payload; rawPayload != nil {
			rctx.Payload = rawPayload.(*RateObservationPayload)
		} else {
			return goa.MissingPayloadError()
		}
		return ctrl.Rate(rctx)
	...
	h = handleObservationOrigin(h)
	service.Mux.Handle("PUT", "/nosh/patients/:patientID/observation/:observationID/actions/rate", ctrl.MuxHandler("Rate", h, unmarshalRateObservationPayload))
...
// unmarshalRateObservationPayload unmarshals the request body into the context request data Payload field.
func unmarshalRateObservationPayload(ctx context.Context, service *goa.Service, req *http.Request) error {
	payload := &rateObservationPayload{}
	if err := service.DecodeRequest(req, payload); err != nil {
		return err
	}
	if err := payload.Validate(); err != nil {
