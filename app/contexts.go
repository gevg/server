...
// Validate runs the validation rules defined in the design.
func (payload *rateObservationPayload) Validate() (err error) {
	if payload.Rating == nil {
		err = goa.MergeErrors(err, goa.MissingAttributeError(`raw`, "rating"))
	}
	return
...
// Publicize creates RateObservationPayload from rateObservationPayload
func (payload *rateObservationPayload) Publicize() *RateObservationPayload {
	var pub RateObservationPayload
	if payload.Rating != nil {
		pub.Rating = *payload.Rating
	}
	return &pub
...
// OKTiny sends a HTTP response with status code 200.
func (ctx *ListPatientContext) OKTiny(r PatientTinyCollection) error {
	ctx.ResponseData.Header().Set("Content-Type", "application/vnd.patient+json; type=collection")
	if r == nil {
		r = PatientTinyCollection{}
	}
	return ctx.ResponseData.Service.Send(ctx.Context, 200, r)
}
...
// OKTiny sends a HTTP response with status code 200.
func (ctx *ShowPatientContext) OKTiny(r *PatientTiny) error {
	ctx.ResponseData.Header().Set("Content-Type", "application/vnd.patient+json")
	return ctx.ResponseData.Service.Send(ctx.Context, 200, r)
}
