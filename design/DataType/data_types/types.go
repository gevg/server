...
var Signature = Type("Signature", func() {
	Description(`A digital signature along with supporting context. The signature may be electronic/cryptographic in nature, or a graphical image
	representing a hand-written signature, or a signature process. Different Signature approaches have different utilities.`)
	Required("contentType")
	//Comments:
	//Reason for inclusion or contrainment:
	Attribute("type", Coding, `An indication of the reason that the entity signed this document. This may be explicitly included as part of the signature
	information and can be used when determining accountability for various actions concerning the document. See http://hl7.org/fhir/ValueSet/signature-type`, func() {
		//Comments:
		//Reason for inclusion or contrainment:
	})
	Attribute("when", DateTime, "A reference to an application-usable description of the person that signed the certificate (e.g. the signature used their private key.", func() {
		//Type: instant
		//Comments:
		//Reason for inclusion or contrainment:
	})
	Attribute("whoUri", String, "A reference to an application-usable description of the person that signed the certificate (e.g. the signature used their private key).", func() {
		//Type: uri
		//Comments:
		//Reason for inclusion or contrainment:
	})
	Attribute("whoReference", HL7Reference, " A reference to an application-usable description of the person that signed the certificate (e.g. the signature used their private key).", func() {
		//Referece: Practioner, RelatedPerson, Patient, Device, Organization
		//Comments:
		//Reason for inclusion or contrainment:
	})
	Attribute("contentType", String, `A mime type that indicates the technical format of the signature. Important mime types are application/signature+
		for X ML DigSig, application/jwt for JWT, and image/* for a graphical image of a signature. See http://www.rfc-editor.org/bcp/bcp13.txt`, func() {
		//Type: code
		//Comments:
		//Reason for inclusion or contrainment:
	})
	Attribute("blob", String, "The base64 encoding of the Signature content.", func() {
		//Type: base64Binary
		//Comments:
		//Reason for inclusion or contrainment:
	})
